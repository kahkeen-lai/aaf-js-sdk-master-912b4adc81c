...
import mockXaabaEngageTimeLoginResponse from '../../apis/login-200-response';
...
import { convertKeyValuePairsStringToMap, setMockServerResponse } from '../../utils';
jest.unmock('axios');
...
describe('AAF JS SDK', () => {
  ...
  it(
    `adStartDelayHint=TRUE;positiveEngageTime 
        expect delay between HOST_AD_INT and AD_INIT`,
    done => {
      const xaafJsSdk = given_mockedClientAndBE_withPositiveDelayedEngagment();
      xaafJsSdk.xaafInitListener = (xaafEvent: XaafEvent) => {
        ...
        _executableAd.executableAdEventListener = async (adEvent: AdEvent) => {
          if (adEvent.type === AdEventType.Loaded) {
            await then_adInitLifecycleEvents_areReportedWithDelay();
            done();
          }
        };
  ...
  it(
    `adStartDelayHint is TRUE , negativeDelayTime 
    expect no delay between HOST_AD_INIT and AD_INIT`,
    done => {
      const xaafJsSdk = given_mockedClientAndBE_withNegativeDelayedEngagment();
      xaafJsSdk.xaafInitListener = (xaafEvent: XaafEvent) => {
        if (isXaafEventTypeNotSuccess(xaafEvent)) {
          fail(`AAF event type is ${xaafEvent.type}`);
        }
        ...
        _executableAd.executableAdEventListener = async (adEvent: AdEvent) => {
          if (adEvent.type === AdEventType.Loaded) {
            await then_adInitLifecycleEvents_areReportedWithoutDelay();
            done();
          }
        };
  ...
  function given_mockedClientAndBE_withNegativeDelayedEngagment(): XaafJsSdk {
    _mockedInitAdinfo.set('adStartDelayHint', '4000');

    const xaafJsSdk = setMockedCientAndBE();
    return xaafJsSdk;
  }

  function given_mockedClientAndBE_withPositiveDelayedEngagment(): XaafJsSdk {
    _mockedInitAdinfo.set('adStartDelayHint', '6000');

    const xaafJsSdk = setMockedCientAndBE();
    return xaafJsSdk;
  }

  function setMockedCientAndBE(): XaafJsSdk {
    const enabledFlagNames: string[] = [FlagName.xaafEnabled, FlagName.adStartHintEnabled];
    const xaafJsSdk = given_xaafJsSdk_withMockedDelegateDependencies(enabledFlagNames);
    setMockServerResponse(
      [mockXaabaEngageTimeLoginResponse, mockOpportunityResponse],
  ...
  function when_initializeIsExecuted_onXaafJsSdk(xaafJsSdk: XaafJsSdk): void {
    xaafJsSdk.initialize(IntegrationApiKeyConfig.tlvDevFiretvApiKey, _mockedSDKArguments);
  }
  ...
  async function then_adInitLifecycleEvents_areReportedWithoutDelay() {
    const delay: boolean = false;
    await then_AdInit_isReportCorrectlyToNR(delay);
  }

  async function then_adInitLifecycleEvents_areReportedWithDelay() {
    const delay: boolean = true;
    await then_AdInit_isReportCorrectlyToNR(delay);
  }

  async function then_AdInit_isReportCorrectlyToNR(expecteDelay: boolean) {
    const nrQuery = getNRQueryByDeviceUUID(_mockedSDKArguments.get('deviceUUID'));
    await queryAndStoreNRReportData(nrQuery);
    ...
    const nrAdInitEventData = getNRAdInitEventData(nrEventsData);
    then_nrAdInitEvent_timeSinceLastLifeCycleEvent_shouldContainCorrectDelay(
      nrAdInitEventData.timeSinceLastLifeCycleEvent,
      expecteDelay
    );
  }
  ...
  async function queryAndStoreNRReportData(nrQuery: string): Promise<void> {
    const newRelicOptions = {
      accountId: IntegrationNRConfig.tlvDevFiretvAccountId,
      queryKey: IntegrationNRConfig.tlvDevFiretvQueryKey
    };
  ...
  function getNRAdInitEventData(nrEventData: IEvent[]): IEvent {
    const adInitEvent = nrEventData.filter(event => event.name === 'AD_INIT');
    return adInitEvent[0];
  }
  ...
  function then_nrAdInitEvent_timeSinceLastLifeCycleEvent_shouldContainCorrectDelay(
    actualNREventTimeSinceLastLifeCycleFieldValue: number,
    expectedDelay: boolean
  ) {
    const preAdStartXaabaEngageTime =
      mockXaabaEngageTimeLoginResponse.data.configuration.pre_ad_start_xaaba_engage_time;
    const timeBetweenEngageTimeAndDelayHint = _mockedInitAdinfo.get('adStartDelayHint') - preAdStartXaabaEngageTime;

    if (expectedDelay) {
      expect(actualNREventTimeSinceLastLifeCycleFieldValue).toBeGreaterThan(timeBetweenEngageTimeAndDelayHint);
    } else {
      expect(actualNREventTimeSinceLastLifeCycleFieldValue).toBeGreaterThanOrEqual(0);
    }
  }
